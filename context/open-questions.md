...
- **Status:** open — reachability is settled by reasoning, not pending a test; a concurrency test at READ COMMITTED (MSSQL or the generic floor, not Postgres, which would have to force isolation artificially) belongs with the fix, to pin the regression
