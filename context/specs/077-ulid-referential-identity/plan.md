...
3. **Writes** (`writes.py`) — snapshot selects `entry_id` (never `id`);
   `_entry_values` consumes `staged.entry_id`; the parent's identity is
   an upfront staged-overlay lookup (landed as `_parent_id`, since moved
   to `WritePlan.parent_id_of` in the review hardening); `_upsert_layer` RETURNING = `(entry_id, path, version)`
...
4. **Reads** (`reads.py`) — anchor/identity column and content join on
   `entry_id`; `ls` re-keyed to `entry_id` anchors and batched into
   chunked `parent_id IN` children selects regrouped per parent (one
   SELECT per anchor before). `engine.py` provisions the root with
   `entry_id`.
