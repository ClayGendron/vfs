...
| P15 | the split sub-batcher loses its trailing batch (`if batch: yield batch` deleted) — the last sub-batch's files silently emit no chunk rows | `src/vfs/storage/backends/database/indexing.py` | `tests/storage/database/test_indexing.py` | `TestSplitBatchBudget::test_sub_batched_splits_match_the_whole_batch_answer` (1 failure, proven 2026-08-25) | spec 120 |
