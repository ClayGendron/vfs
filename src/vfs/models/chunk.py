...
:meth:`Chunk.split_batch` is the production splitter: it dispatches each row's
split by file extension (notebooks, tree-sitter grammars, recursive
separators) and enumerates the resulting chunks in order. :meth:`Chunk.split`
is the single-item convenience over the same door.
...
class Chunk(BaseModel):
    ...
    def split(cls, *, file: Path, content: str, ext: str | None) -> list[Chunk]:
        """:meth:`split_batch` for one ``(file, content, ext)`` row.

        Dispatches on the file extension *ext* (no leading dot): notebooks use
        ``split_notebook``, extensions with a tree-sitter grammar use the
        structure-aware splitter, and everything else falls back to the
        ...
        """
        return cls.split_batch([(file, content, ext)])[0]

    @classmethod
    def split_batch(cls, files: Sequence[tuple[Path, str, str | None]]) -> list[list[Chunk]]:
        """Ordered chunks for many ``(file, content, ext)`` rows at once.

        The production splitter: grammar-routed files cross the engine
        seam in a single batch call, so the native engine parses them in
        parallel; notebooks and fallback extensions split per row.
