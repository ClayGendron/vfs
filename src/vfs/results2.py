...
# ---------------------------------------------------------------------------
# Error vocabulary and contract tables
# ---------------------------------------------------------------------------
...
class VFSErrorKind(StrEnum):
    ...
    string by ``ResultError`` and handled at the broadest tier — the
    boundary adapter raises it as base ``VFSError``, never a narrower
    class that would misstate an unknown failure.
...
class Result(BaseModel):
    ...
    - **Envelope:** ``ops`` records every op whose rows were merged in
      ...
      arrangement on ``None``. Inbound payloads carrying a legacy
      ``function`` string are shimmed to a one-element tuple.
    ...
    @classmethod
    def _shim_envelope(cls, data: Any) -> Any:
        """Drop any stored verdict; shim the legacy ``function`` string.
        ...
        if isinstance(data, dict):
            ...
            if function and "ops" not in data:
                data["ops"] = (function,)
        return data
    ...
    def function(self) -> str | None:
        """Transitional alias for :attr:`op` — render/projection still
        read it; delete once they dispatch on ``op``."""
        return self.op
    ...
    def _demoted(self) -> Result:
        ...
        errors = [
            e.model_copy(update={"severity": Severity.warning}) if e.is_fatal else e
...
_QUARANTINE_CLIP = 500  # bound raw peer junk carried in quarantine records
