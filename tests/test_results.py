"""Tests for VFS result types — Candidate, LineMatch, VFSResult."""
...
from vfs.results import Candidate, LineMatch, VFSResult, _format_field, _verb_for
...
class TestEntry:
    def test_construction(self):
        e = Candidate(path="/src/auth.py", kind="file")
    ...
    def test_name_property(self):
        assert Candidate(path="/src/auth.py").name == "auth.py"
        assert Candidate(path="/.vfs/a.py/__meta__/versions/3").name == "3"

    def test_with_line_matches(self):
        lm = LineMatch(start=1, end=3, match=2)
        e = Candidate(path="/a.py", lines=[lm])
    ...
    def test_zero_metrics_preserved_in_json(self):
        ...
        e = Candidate(path="/a.py", size_bytes=0, score=0.0)
        data = e.model_dump(exclude_none=True)
        assert data["size_bytes"] == 0
        assert data["score"] == 0.0

    def test_json_round_trip(self):
        e = Candidate(path="/a.py", kind="file", size_bytes=50, score=0.42)
        data = e.model_dump()
...
class TestVFSResultBasics:
    ...
    def test_failed_result_is_falsy(self):
        r = VFSResult(success=False, function="glob", candidates=[_e("/a.py")])
        assert not r

    def test_contains(self):
        r = VFSResult(function="glob", candidates=[_e("/a.py")])
        assert "/a.py" in r
        assert "/b.py" not in r

    def test_iter_entries(self):
        entries = [_e("/a.py"), _e("/b.py")]
        r = VFSResult(function="glob", candidates=entries)
        paths = [e.path for e in r.iter_candidates()]
        assert paths == ["/a.py", "/b.py"]

    def test_content_shorthand(self):
        r = VFSResult(function="read", candidates=[_e("/a.py", content="print('hello')")])
...
def _make(paths: list[str], function: str = "glob", score: float | None = None) -> VFSResult:
    entries = [Candidate(path=p, kind="file", score=score) for p in paths]
...
class TestVFSResultSetAlgebra:
    ...
    def test_success_propagation_and(self):
        a = VFSResult(success=True, function="glob", candidates=[_e("/a.py")])
        b = VFSResult(success=False, function="glob", candidates=[_e("/a.py")])
        result = a & b
        assert result.success is False

    def test_success_propagation_or(self):
        a = VFSResult(success=True, function="glob", candidates=[_e("/a.py")])
        b = VFSResult(success=False, function="glob", candidates=[_e("/b.py")])
        result = a | b
        assert result.success is False
...
class TestLeftWinsMerge:
    def test_intersection_left_wins_on_content(self):
        a = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file", content="from left")],
        )
        b = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file", content="from right")],
        )
        result = a & b
        assert result.candidates[0].content == "from left"

    def test_merge_preserves_empty_string_content(self):
        """content='' (empty file) should NOT be replaced by right's content."""
        a = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file", content="")],
        )
        b = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file", content="real content")],
        )
        result = a & b
    ...
    def test_merge_preserves_zero_metrics_from_left(self):
        ...
        a = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file", size_bytes=0)],
        )
        b = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file", size_bytes=4096)],
        )
        result = a & b
    ...
    def test_as_dict_last_wins_on_duplicate_paths(self):
        ...
        e1 = Candidate(path="/a.py", kind="file", content="first")
        e2 = Candidate(path="/a.py", kind="file", content="second")
        r = VFSResult(function="glob", candidates=[e1, e2])
        d = r._as_dict()
        assert len(d) == 1
...
class TestFunctionPropagation:
    ...
    def test_empty_envelope_union_with_empty_envelope(self):
        ...
        a = VFSResult(candidates=[_e("/a.py")])
        b = VFSResult(candidates=[_e("/b.py")])
        result = a | b
...
class TestVFSResultEnrichment:
    def test_sort_by_score_default(self):
        r = VFSResult(
            ...
            candidates=[
                Candidate(path="/low.py", score=0.1),
                Candidate(path="/high.py", score=0.9),
                Candidate(path="/mid.py", score=0.5),
            ],
        )
        sorted_r = r.sort()
        assert [e.path for e in sorted_r.iter_candidates()] == ["/high.py", "/mid.py", "/low.py"]
    ...
    def test_sort_ascending(self):
        ...
        assert [e.path for e in sorted_r.iter_candidates()] == ["/low.py", "/high.py"]
    ...
    def test_sort_none_scores_sink_to_bottom(self):
        r = VFSResult(
            function="vector_search",
            candidates=[
                Candidate(path="/a.py", score=None),
                Candidate(path="/b.py", score=0.5),
            ],
        )
        sorted_r = r.sort()
        assert sorted_r.candidates[0].path == "/b.py"
        assert sorted_r.candidates[1].path == "/a.py"

    def test_sort_preserves_function(self):
        r = VFSResult(
            function="vector_search",
            candidates=[Candidate(path="/a.py", score=0.1)],
        )
    ...
    def test_top(self):
        r = VFSResult(
            function="vector_search",
            candidates=[
                Candidate(path="/a.py", score=0.1),
                Candidate(path="/b.py", score=0.9),
                Candidate(path="/c.py", score=0.5),
            ],
        )
        top2 = r.top(2)
        assert len(top2) == 2
    ...
    def test_top_more_than_available(self):
        r = VFSResult(function="vector_search", candidates=[Candidate(path="/a.py", score=0.1)])
        top5 = r.top(5)
        assert len(top5) == 1

    def test_top_zero_raises(self):
        r = VFSResult(function="vector_search", candidates=[Candidate(path="/a.py", score=0.5)])
    ...
    def test_kinds(self):
        r = VFSResult(
            ...
            candidates=[
                ...
                Candidate(path="/b/", kind="directory"),
                Candidate(path="/.vfs/a.py/__meta__/chunks/login", kind="chunk"),
            ],
        )
    ...
    def test_add_prefix_empty_is_noop(self):
        r = VFSResult(function="glob", candidates=[Candidate(path="/a.py")])
        r.add_prefix("")
        assert r.paths == ("/a.py",)
    ...
    def test_strip_user_scope(self):
        r = VFSResult(
            function="glob",
            candidates=[Candidate(path="/123/docs/README.md"), Candidate(path="/123/src/a.py")],
        )
...
class TestVFSResultJSON:
    def test_to_json_excludes_none_by_default(self):
        r = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file")],
        )
    ...
    def test_to_json_round_trip(self):
        r = VFSResult(
            ...
            function="glob",
            candidates=[Candidate(path="/a.py", kind="file"), Candidate(path="/b.py", kind="file")],
        )
        json_str = r.to_json()
    ...
    def test_model_dump_round_trip(self):
        r = VFSResult(
            function="vector_search",
            candidates=[Candidate(path="/a.py", score=0.9), Candidate(path="/b.py", score=0.5)],
        )
        data = r.model_dump()
        restored = VFSResult.model_validate(data)
        assert restored.function == "vector_search"
        assert [e.score for e in restored.candidates] == [0.9, 0.5]
...
class TestToStr:
    def test_glob_one_path_per_line(self):
        r = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a.py"), Candidate(path="/b.py"), Candidate(path="/c.py")],
        )
        rendered = r.to_str()
    ...
    def test_markdown_table_pipes_and_dashes_align(self):
        ...
        r = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a", size_bytes=1), Candidate(path="/long_path", size_bytes=9999)],
        )
    ...
    def test_markdown_table_right_aligns_numeric_fields(self):
        ...
        r = VFSResult(
            function="glob",
            candidates=[Candidate(path="/a", size_bytes=5)],
        )
        rendered = r.to_str(projection=("path", "size_bytes"))
    ...
    def test_markdown_table_escapes_pipes_in_values(self):
        ...
        r = VFSResult(
            function="glob",
            candidates=[Candidate(path="/has|pipe", kind="file")],
        )
    ...
    def test_empty_multicolumn_still_emits_header_and_divider(self):
        ...
        r = VFSResult(function="glob", candidates=[])
        rendered = r.to_str(projection=("path", "size_bytes"))
    ...
    def test_grep_renders_path_line_content(self):
        ...
        r = VFSResult(function="grep", candidates=[e])
        rendered = r.to_str()
    ...
    def test_explicit_null_projection_appends_note(self):
        ...
        assert "NOTE: out_degree not populated for any candidates." in rendered
    ...
    def test_grep_native_projection_still_renders_line_by_line(self):
        ...
        e = Candidate(
            ...
            content=content,
            lines=[LineMatch(start=1, end=3, match=2)],
            # Candidate has entry-level data but we don't project it — stay rg-style.
            size_bytes=47,
        )
        r = VFSResult(function="grep", candidates=[e])
    ...
    def test_grep_no_segments_emits_path(self):
        ...
        e = Candidate(path="/a.py", content="hit\n")
        r = VFSResult(function="grep", candidates=[e])
    ...
    def test_vector_search_renders_as_markdown_table(self):
        r = VFSResult(
            function="vector_search",
            candidates=[Candidate(path="/a.py", score=0.9)],
        )
        rendered = r.to_str()
    ...
    def test_read_returns_content(self):
        r = VFSResult(
            function="read",
            candidates=[Candidate(path="/a.py", content="print('hi')")],
        )
