"""Tests for text rendering of VFSResult — per-function arrangements."""
...
class TestErrorRendering:
    ...
    def test_unpopulated_projection_note(self) -> None:
        result = VFSResult(function="glob", observations=[obs("/a.md")])
    ...
    def test_note_fires_for_grep_when_matches_and_content_absent(self) -> None:
        result = VFSResult(function="grep", observations=[obs("/a.py")])
    ...
    def test_note_suppressed_for_renderers_that_ignore_projection(self) -> None:
        tree = VFSResult(function="tree", observations=[obs("/d/a.py")])
        assert "NOTE:" not in tree.to_str(projection=("path", "score"))
        action = VFSResult(function="delete", observations=[obs("/d/a.py")])
...
class TestPathListAndTable:
    def test_path_only_projection_is_sorted_lines(self) -> None:
        result = VFSResult(function="ls", observations=[obs("/b.md"), obs("/a.md")])
        assert str(result) == "/a.md\n/b.md"
    ...
    def test_unknown_function_falls_back_to_path_list(self) -> None:
        result = VFSResult(function="future_op", observations=[obs("/a.md")])
        assert str(result) == "/a.md"
...
class TestGrepRendering:
    ...
    def test_files_with_matches_mode_renders_paths(self) -> None:
        result = VFSResult(function="grep", observations=[obs("/a.py"), obs("/b.py")])
...
class TestReadStatTree:
    def test_read_single_renders_content_verbatim(self) -> None:
        result = VFSResult(function="read", observations=[obs("/a.md", content="# Title\nbody")])
...
class TestActionRendering:
    def test_single_path_action(self) -> None:
        result = VFSResult(function="delete", observations=[obs("/a.md")])
        assert str(result) == "Deleted /a.md"

    def test_multi_path_action(self) -> None:
        result = VFSResult(function="move", observations=[obs("/a.md"), obs("/b.md")])
        assert str(result) == "Moved 2 paths"

    def test_remaining_action_verbs(self) -> None:
        assert str(VFSResult(function="copy", observations=[obs("/a.md")])) == "Copied /a.md"
        assert str(VFSResult(function="mkdir", observations=[obs("/d")])) == "Created /d"
        assert str(VFSResult(function="mkedge", observations=[obs("/a.md")])) == "Connected /a.md"
    ...
    def test_empty_action(self) -> None:
        assert str(VFSResult(function="edit")) == "No changes"

    def test_write_empty_is_nothing_to_do(self) -> None:
        assert str(VFSResult(function="write")) == "write: nothing to do"
    ...
    def test_write_single_file_detail_line_uses_status(self) -> None:
        result = VFSResult(function="write", observations=[obs("/b.md", kind="file", status="updated")])
        assert str(result) == "write success: 1 file\n\n  updated /b.md"

    def test_write_single_file_without_status_reads_as_created(self) -> None:
        result = VFSResult(function="write", observations=[obs("/a.md", kind="file")])
        assert str(result) == "write success: 1 file\n\n  created /a.md"
